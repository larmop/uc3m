...
#xgb_tune <- train(Model_forw, data = cineTrain_small,
                 # method = "xgbTree",
                  #preProc=c('scale','center'),
...
# 
# results10 <- c("Gradient Boosting", x[1], x[2], x[3])
# 
# # Adding these results to the results table
...
Finally, we will plot the importance of each variable:
...
Anyway, let's just see the results:
...
```{r}
yhat = exp(test_results$bw)

head(yhat) # show the prediction for 6 films screened in a cinema

hist(yhat, col="lightblue")

prediction_inter <- predict(back_tune, cineTest_small, interval = "prediction") # Assuming prediction_inter contains the lower and upper prediction intervals

lower_bound <- exp(prediction_inter[, "lwr"])
upper_bound <- exp(prediction_inter[, "upr"])

# Creating a data frame with predictions, lower, and upper bounds
prediction_df <- data.frame(
  yhat = exp(prediction_inter[, "fit"]),
  lower = lower_bound,
  upper = upper_bound
)
hist(prediction_df$yhat, col = "lightblue", main = "Histogram of Predictions")

cineTest_small$Pred = exp(prediction_inter[, "fit"])
cineTest_small$Real = y
cineTest_small$Lower = exp(prediction_inter[, "lwr"])
cineTest_small$Upper = exp(prediction_inter[, "upr"])  # Prediction Interval Plot

cineTest_small %>% arrange(total_sales) %>% ggplot(aes(x = total_sales, y = y)) +   geom_point() +   geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = 0.2) +   labs(title = "Prediction Intervals", y = "Real Values") +   theme_minimal()  
```

### 7.2 Final predictions and intervals for random forests

```{r}
yhat = exp(test_results$rf)
