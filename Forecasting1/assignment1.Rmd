...
#### 3.1.1 Time-Series decomposition and seasonal adjustment
...
We apply seasonal adjustment here, to show how the series looks after adjusting and removing seasonality.

```{r}

```

3.  **Cycles:** In the previous plots we have seen that there is not a clear trend, but a cycle, as it magnitude is variable and the timing of peaks and troughs is unpredictable in the long term and not of a fixed period.
...
Now, we have to decide whether we will forecast based on the day-of-week or the time-of-year effects. We will develop a 1-day-ahead forecasting model and a 7-day ahead forecasting model.
...
```{r}
website_ts |> gg_season(unique_visits, period = "month")
...
Decide if we will predict for next day, month, etc: If we predict what will happen tomorrow respect to today cor (Yt, Yt-1)\--\> autocorrelation of order 1

cor (Yt, Yt-2) \--\> order 2

This order (the amount of info from the past needed to predict) is a hyperparameter of the model.
...
If the trend is already linearly increasing, don't perform any transformation to the data.

I want to predict in the short term so try to find a trend in the short term --\> 1 day ahead or 7 days ahead

## 4. Statistical Analysis (tests, ACF, PACF, performance, etc.). Looking for the best model.

### Autocorrelations and lags --\> Also take dfferences here to decide ARIMAs
...
The first bar represents the correlation between a period and the previous one.

The negatives are consequence of the positives.

If the correlation is inside the blue area, it can be said that it is zero.

Positive bars are decreasing. depending on the speed they're decreasing at, we should use faster or slower models.

If it is similar to a line \--\> slow

If it is almost a curve \--\> Fast
...
When data have a trend, the auto correlations for small lags tend to be large and positive

\- When data are seasonal, the auto correlations will be larger at the seasonal lags (i.e., at multiples of the seasonal frequency)
...
Useful by-product of decomposition: an easy way to calculate seasonally adjusted data. See whether the number of unique visits is changing removing the season.

Additive decomposition: seasonally adjusted data given by (removing the seasonality) --\> y_t - S_t = T_t + R_t

Multiplicative decomposition: seasonally adjusted data given by: y_t / S_t = T_t \\times R_t

Shows how the series looks after adjusting and removing seasonality. Seasonal components have been removed:

```{r}
website_ts |>
  autoplot(unique_visits, color = "gray") +
  autolayer(components(dcmp), season_adjust, color = "#0072B2") +
  labs(y = "Unique visits", title = "Total visits")
```
...
STL (Seasonal Decomposition of Time Series):] extracts trend, seasonal, with locally smoothed moving average
