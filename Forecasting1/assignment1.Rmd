...
Now, we have to decide whether we will forecast based on the day-of-week or the time-of-year effects on this daily data. We will predict in the short term by developing a 1-day-ahead forecasting horizon model and a 7-day ahead one. Because of that, we will be calculating the correlation btetween Yt and Yt-1, an autocorrelation of order 1. This order (the amount of info from the past needed to predict) will be a hyper parameter of the model.

In the short term the trend is already linearly increasing, so we will not perform any transformation to the data.
